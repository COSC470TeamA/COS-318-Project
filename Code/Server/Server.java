...
public class Server {
    ...
    protected static byte[] _LOBBY_STATE = new byte[8]; //Lobby for each of the lobbies, needs to be its own class one day
    //protected GAME gameArray[8]; 
    protected static GameLobby[] gs = new GameLobby[8]; //gamestate for each of the games, mapped to the lobby.
    ...
    static int[] xCoord = new int[8],
            yCoord = new int[8];
    static char[] charVal = new char[6];
    ...
    protected static void newGame(int lobbyNum) {
        ...
        gs[lobbyNum] = new GameLobby(lobbyNum);
        _GAMES_PLAYED++;
    }
}
